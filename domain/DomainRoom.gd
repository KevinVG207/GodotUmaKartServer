...
class Room:
	...
	var pingData: Dictionary[int, PingData] = {}
	...
	func _on_player_join_room(player: DomainPlayer.Player) -> void:
		pingData[player.peer_id] = PingData.new()
		return
	
	func _on_player_leave_room(player: DomainPlayer.Player) -> void:
		pingData.erase(player.peer_id)
		return
	...
	func ping_players() -> void:
		...
		for pid: int in players:
			...
			pingData[pid].ongoing_pings.append(cur_tick)
			RPCClient.receive_ping.rpc_id(pid, cur_tick)
	
	func handle_ping(pid: int, org_tick: int) -> void:
		if not pid in players:
			return
		if not org_tick in pingData[pid].ongoing_pings:
			return
		var cur_tick := Time.get_ticks_msec()
		var one_way := roundi((cur_tick - org_tick) / 2.0)
		pingData[pid].ongoing_pings.erase(org_tick)
		pingData[pid].last_pings.append(one_way)
		while pingData[pid].last_pings.size() > PingData.max_pings:
			pingData[pid].last_pings.pop_front()
		var avg_ping := 0.0
		for ping in pingData[pid].last_pings:
			avg_ping += ping
		avg_ping /= pingData[pid].last_pings.size()
